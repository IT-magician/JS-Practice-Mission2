class TimeManager {
  ...
  multiplerIncrease(uuid, value = 2) {
    target = this.pungs.find((item) => item.uuid === uuid)
  ...
  Stop(uuid) {
    this.pungs.find((item) => item.uuid === uuid)?.isStop = true;
  }
  ...
  Start(uuid) {
    this.pungs.find((item) => item.uuid === uuid)?.isStop = false;
  }
  ...
  Increase(value = 5) {
    this.pungs.find((item) => item.uuid === uuid)?.value += value
}
