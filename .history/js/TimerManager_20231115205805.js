class TimerManager {
  ...
  }, this.#renderCallInterval);

  #viewRender;

  constructor(
    viewRender,
    renderCallInterval = 1000,
    cmp = (lhs, rhs) => rhs.value - lhs.value
  ) {
    this.#viewRender = viewRender;
    this.#renderCallInterval = renderCallInterval;
    this.#cmp = cmp;
  }
  ...
  TimerInfoClass = class {
     ...
     */
    Multipler = (value = 2 * 1000) => (this.value *= value);
    Add = (value = 5 * 1000) => (this.value += value);
  ...
  GetAverage() {
    if (this.#pungs.length == 0) return 0;

    return this.#totalSum / 1000 / this.#pungs.length;
  }

  Create(name, value) {
    let timerInfo = this.#append(name, value);
    const uuid = timerInfo.uuid;
    return;
    new Promise((resolve, reject) => {
      ...
      timerInfo.timer = setInterval(() => {
        ...
        this.#totalSum -= this.#getDiff(past, timerInfo.value); // down value
      }, this.#checkInterval);
    }).then((resolve, reject) => {
      // 종료 작업
      this.Cancel(uuid);
  ...
  multiplerIncreaseAll(value = 2 * 1000) {
    const previousAllStopStatus = this.#isAllStop;
    this.StopAll();
    ...
    // this.#pungs.sort(this.#cmp);

    if (previousAllStopStatus === false) this.StartAll();
  }

  multiplerIncrease(uuid, value = 2 * 1000) {
    this.#pungs
  ...
  Stop(uuid) {
    console.log(this.#pungs.find((item) => item.uuid === uuid));
